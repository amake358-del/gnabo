...
export function Badge({ variant = 'default', children, dot }) {
    return (_jsxs("span", { className: `inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${variants[variant]}`, children: [dot && _jsx("span", { className: `w-1.5 h-1.5 rounded-full ${dotColors[variant]}` }), children] }));
}
export function StatutBadge({ statut }) {
    ...
    return _jsx(Badge, { variant: s.variant, dot: true, children: s.label });
}
