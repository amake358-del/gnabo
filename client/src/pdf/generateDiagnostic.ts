...
import type { Settings, Appareil, Diagnostic } from '../types'
...
function drawDefautCard(doc: jsPDF, appareil: Appareil, y: number): number {
  const cardH = 36
...
function drawDiagnosticContent(doc: jsPDF, diagnostic: Diagnostic, cout_estime: number, currency: string, y: number): number {
...
export async function generateDiagnosticPdf(data: DiagnosticData, settings: Settings): Promise<Blob> {
