...
function currency(s) { return s.currency || ''; }
async function drawHeader(doc, settings, title, numero, statut, y) {
    ...
    doc.setFont(FONT, 'bold');
    doc.setFontSize(5.5);
    doc.setTextColor(statut === 'payee' || statut === 'accepté' ? C.success : statut === 'refusé' || statut === 'annulee' ? C.danger : C.warning);
    const sepY = y + 33;
    doc.setDrawColor(C.border);
    doc.setLineWidth(0.3);
...
function drawClientInfo(doc, client, y) {
...
function drawTable(doc, lines, y) {
    const cols = [
        { x: M, w: CW * 0.45, align: 'left' },
        { x: M + CW * 0.45, w: CW * 0.1, align: 'center' },
        { x: M + CW * 0.55, w: CW * 0.22, align: 'right' },
        { x: M + CW * 0.77, w: CW * 0.23, align: 'right' },
    ];
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        const l = lines[idx];
        if (yy + rowH > PAGE_H - 20) {
            doc.addPage();
            yy = M;
        }
        if (idx % 2 === 0)
            fillRoundRect(doc, M, yy - 1.5, CW, rowH, 2, C.grayRow);
        doc.setFont(FONT, 'normal');
        doc.setFontSize(7);
        doc.setTextColor(C.text);
...
function drawFooter(doc, notes, y, settings) {
    if (notes) {
        doc.setFont(FONT, 'bold');
        doc.setFontSize(7);
        doc.setTextColor(C.text);
...
export async function generateDevisElectroniquePdf(devis, settings) {
...
export async function generateFactureElectroniquePdf(facture, settings) {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
...
export async function generateRecuPdf(paiement, settings) {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
