...
function drawSummary(doc: jsPDF, data: RapportCaisseData, y: number): number {
...
function drawMouvementsTable(doc: jsPDF, entries: CaisseEntry[], y: number, curr: string): number {
  ...
  const aligns: ('left' | 'right')[] = ['left', 'left', 'left', 'left', 'left', 'right']
  headers.forEach((h, i) => {
    const a = aligns[i]
...
export async function generateRapportCaissePdf(data: RapportCaisseData): Promise<Blob> {
