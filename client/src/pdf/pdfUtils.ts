...
export function currency(s: Settings): string {
  return s.currency || ''
}
...
export function fillRoundRect(doc: jsPDF, x: number, y: number, w: number, h: number, r: number, color: string) {
  doc.setFillColor(color)
  doc.roundedRect(x, y, w, h, r, r, 'F')
}

export function companyColor(s: Settings): string {
  return s.service_color || C.accent
}

export async function drawHeader(doc: jsPDF, settings: Settings, title: string, numero: string, y: number): Promise<number> {
...
export function drawClientCard(doc: jsPDF, client: { nom?: string; telephone?: string; adresse?: string; company?: string }, y: number): number {
  const cardH = 28
...
export function drawFooter(doc: jsPDF, pageCount: number, settings: Settings) {
...
export function drawSignatureArea(doc: jsPDF, y: number, leftLabel: string, rightLabel: string): number {
  const colW = (CW - 10) / 2
