...
import type { Devis, Settings } from '../types'
...
function currency(s: Settings): string {
  return s.currency || ''
}

function companyColor(s: Settings): string {
...
function fmtDecimal(n: number, d = 2): string {
...
function drawBadge(doc: jsPDF, label: string, color: string, x: number, y: number, w?: number) {
...
async function drawHeader(doc: jsPDF, devis: Devis, settings: Settings, y: number): Promise<number> {
...
function drawClientCard(doc: jsPDF, devis: Devis, y: number): number {
  const cardH = 34
  const cardY = y
  ...
  doc.setTextColor(C.textLight)
  let cy = cardY + 20.5
  if (addr) { doc.text(addr, M + 12, cy); cy += LH + 0.5 }
  if (tel) { doc.text(tel, M + 12, cy); cy += LH + 0.5 }
  if (email) { doc.text(email, M + 12, cy); cy += LH + 0.5 }
...
function drawNotes(doc: jsPDF, devis: Devis, y: number): number {
  if (!devis.notes) return y
...
const COL_X = (i: number) => M + COL_W.slice(0, i).reduce((a, b) => a + b, 0)
const MIN_ROW_H = 6

function drawTable(doc: jsPDF, devis: Devis, startY: number, settings: Settings): number {
...
function drawFinancialSummary(doc: jsPDF, devis: Devis, y: number, settings: Settings): number {
  const curr = currency(settings)
...
function drawTerms(doc: jsPDF, settings: Settings, y: number): number {
  if (!settings.conditions) return y
...
function drawSignatures(doc: jsPDF, y: number, settings: Settings): number {
...
function drawFooter(doc: jsPDF, pageCount: number, settings: Settings) {
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (settings.address) leftParts.push(settings.address)
    if (settings.phone) leftParts.push(`Tél: ${settings.phone}`)
    if (settings.email) leftParts.push(settings.email)
...
function drawWatermark(doc: jsPDF, devis: Devis, pageCount: number) {
...
function numberToWords(n: number): string {
  ...
  while (n > 0) {
    ...
    if (part > 0) {
      ...
      const rest = part % 100
      if (hundreds > 0) partWords += (hundreds === 1 ? 'cent' : nums[0][hundreds] + ' cent') + ' '
      if (rest > 0) {
        if (rest < 10) partWords += nums[0][rest]
        else if (rest < 20) partWords += nums[1][rest - 10]
        else {
          const tens = Math.floor(rest / 10)
          const units_d = rest % 10
          partWords += nums[2][tens - 2]
          if (units_d > 0) partWords += '-' + nums[0][units_d]
        }
        partWords += ' '
      }
      if (unitIndex > 0) partWords += units[unitIndex] + (part > 1 && unitIndex > 0 ? 's' : '') + ' '
...
function montantEnLettres(montant: number, settings?: Settings): string {
  ...
  if (centimes > 0) result += ` et ${centimes} centimes`
  return result
}

// ── Main ──

export async function generatePdf(devis: Devis, settings: Settings): Promise<Blob> {
