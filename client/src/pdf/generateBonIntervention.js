...
function drawInterventionInfo(doc, intervention, settings, y) {
    const cardH = 38;
...
function drawAdresseChantier(doc, intervention, y) {
    const cardH = 18;
...
function drawDescriptionTravaux(doc, intervention, y) {
...
async function drawPhotos(doc, photos, label, y) {
    if (!photos)
        return y;
...
export async function generateBonInterventionPdf(data) {
...
function drawClientCard(doc, client, y) {
    const cardH = 24;
    fillRoundRect(doc, M, y, CW, cardH, 6, C.lightBg);
    doc.setDrawColor(C.accent);
    doc.setLineWidth(1.8);
